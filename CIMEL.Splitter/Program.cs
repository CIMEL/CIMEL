    ...
    internal class Program
    {
        ...
        private static void Main(string[] args)
        {
            try
            {
                ...
                var cimelFile = ChartMappings.Signleton.CIMELFile;
                cimelFile.Name = chartSetName;
                // relative path
                cimelFile.Path = chartSetName; //chartSetPath;
                ...
                foreach (var pair in ChartMappings.Signleton.GetGroups())
                {
                    ...
                    foreach (var strChartName in arrGroups)
                    {
                        ...
                        // skip the chart mapping if no data file extracted from the cimel inversion file(.dat)
                    ...
                    cimelFile.DataConfigs.Add(string.Format("{0}|{1}", strGroupNames, chartDesc));
                }
                // generate cimel file (.cimel)
                string cimelfile = cimelFile.Save(root, chartSetName);
                OnInformed(string.Format("Saved cimel file -> {0}", cimelfile));
            }
            catch (Exception ex)
            {
